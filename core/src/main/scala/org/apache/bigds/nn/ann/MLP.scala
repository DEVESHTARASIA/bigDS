...
import org.apache.spark.mllib.bigds.optimization.{Gradient, GradientDescent, LBFGS}
...
import org.apache.spark.mllib.bigds.pmml.PMMLExportable
...
class MLP(
  ...
  val dropout: Array[Double]) extends Logging with Serializable with PMMLExportable {
  ...
  protected[mllib] def computeLoss(
    x: SM, label: SM): Double = {
    val batchSize = x.numCols
    val in = new Array[SM](numLayer)
    val out = new Array[SM](numLayer)
    val dropOutMasks: Array[SM] = dropOutMask(batchSize)
    ...
    if (innerLayers.last.layerType == "SoftMax") {
      NNUtil.crossEntropy(out.last, label)
    } else {
      NNUtil.meanSquaredError(out.last, label)
    }
  ...
  protected[ann] def assign(newNN: MLP): MLP = {
...
object MLP extends Logging {
  ...
  def runSGD(
    ...
    runSGD(data, nn, batchSize, maxNumIterations, fraction,
      learningRate, weightCost, 0.99, 0.01)
  }
  ...
  def runSGD(
    ...
    val gradient = new MLPGradient(
      ...
      mlp.dropout,
      batchSize)
    val updater = new MLPAdaGradUpdater(mlp.topology, rho, epsilon)
  ...
  def runLBFGS(
    ...
    weightCost: Double): MLP = {
    val nn = new MLP(topology)
    runLBFGS(trainingRDD, validRDD, nn, batchSize, maxNumIterations,
  ...
  def runLBFGS(
    ...
    val trainingRDD = toTrainingRDD(data)
    val validRDD = toTrainingRDD(valid)
    trainingRDD.persist(StorageLevel.MEMORY_AND_DISK).setName("MLP-dataBatch")
    val weights = optimizer.optimize(trainingRDD, validRDD, toVector(nn))
    trainingRDD.unpersist()
...
private[mllib] class MLPGradient(
  ...
  def computeLoss(
    ...
    iter.map(_._2).grouped(batchSize).foreach { seq =>
      ...
      loss += mlp.computeLoss(Matrices.fromBreeze(input),
        Matrices.fromBreeze(label))
    }
    loss
  }
...
private[mllib] class MLPAdaGradUpdater(
  ...
  epsilon: Double = 1e-2,
  gamma: Double = 1e-1,
  momentum: Double = 0.9D) extends AdaGradUpdater(rho, epsilon, gamma, momentum) {
