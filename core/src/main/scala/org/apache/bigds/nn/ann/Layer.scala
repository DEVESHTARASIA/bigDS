...
private[mllib] trait Layer extends Serializable {
  ...
  def computeNeuronPrimitive(output: SM): SM
...
private[mllib] class SigmoidLayer(
  ...
  override def computeNeuronPrimitive(output: SM): SM = {
    val der = Matrices.zeros(output.numRows, output.numCols)
    for (i <- 0 until output.numRows) {
      for (j <- 0 until output.numCols) {
        der(i, j) = output(i, j) * sigmoidPrimitive(output(i, j))
      }
    }
    der
  }
...
private[mllib] class TanhLayer(
  ...
  def computeNeuronPrimitive(output: SM): SM = {
    Matrices.zeros(output.numRows, output.numCols)
  }
