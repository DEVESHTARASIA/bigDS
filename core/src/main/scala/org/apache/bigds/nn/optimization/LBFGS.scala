...
package org.apache.spark.mllib.bigds.optimization

import org.apache.spark.mllib.bigds.ann.MLPGradient
 ...
 */
@DeveloperApi
class LBFGS(private var gradient: MLPGradient, private var updater: Updater)
   ...
   */
  def setGradient(gradient: MLPGradient): this.type = {
  ...
  def optimize(data: RDD[(Double, Vector)], valid: RDD[(Double, Vector)], initialWeights: Vector): Vector = {
    val (weights, _) = LBFGS.runLBFGS(
      data,
      valid,
      gradient,
...
object LBFGS extends Logging {
   ...
   * @param data - Input data for L-BFGS. RDD of the set of data examples, each of
   *               the form (label, [feature values]).
   * @param valid - Validation data for L-BFGS, RDD of the set of data examples, each of
   *               the form (label, [feature values]).
   * @param gradient - Gradient object (used to compute the gradient of the loss function of
   *                   one single data example)
   * @param updater - Updater function to actually perform a gradient step in a given direction.
  ...
  def runLBFGS(
    ...
    val lossHistory = new ArrayBuffer[Double](maxNumIterations)
    val earlyStoppingHistory = new ArrayBuffer[Double](maxNumIterations)

    val numExamples = data.count()

    val lossFun =
      new LossFun(data, valid, gradient, updater, regParam, numExamples)

    val lbfgs = new BreezeLBFGS[BDV[Double]](maxNumIterations, numCorrections, convergenceTol)

    val states =
      lbfgs.iterations(new CachedDiffFunction(lossFun), initialWeights.toBreeze.toDenseVector)
    ...
    while(states.hasNext) {
      lossHistory.append(state.value)
      earlyStoppingHistory.append(state.loss)
      state = states.next()
    }
    lossHistory.append(state.value)
    earlyStoppingHistory.append(state.loss)
    ...
    logInfo("LBFGS.runLBFGS finished. Last 10 losses of Training Data %s".format(
      lossHistory.takeRight(10).mkString(", ")))

    logInfo("LBFGS.runLBFGS finished. Last 10 losses of Validation Data %s".format(
      earlyStoppingHistory.takeRight(10).mkString(", ")
    ))
  ...
  /**
   * LossFun implements Breeze's DiffFunction[T], which returns the loss and gradient
   * at a particular point (weights). It's used in Breeze's convex optimization routines.
   */
  private class LossFun(
      data: RDD[(Double, Vector)],
      valid: RDD[(Double, Vector)],
      gradient: MLPGradient,
      updater: Updater,
    ...
    // calculateVal returns the loss at a particluar point(weights).
    override def calcalateVal(weights: BDV[Double]): Double = {
      val w = Vectors.fromBreeze(weights)
      val n = w.size
      val bcW = data.context.broadcast(w)
      val localGradient = gradient

      val lossSum = valid.mapPartitions(t => Iterator(t))
        .treeAggregate(0D)(
          seqOp = (c, v) => {
            val l = localGradient.computeLoss(v, bcW.value)
            c + l
          },
          combOp = (c1, c2) => {
            c1 + c2
